...
// CLI mode (#41): same core as the MCP tools, but stdout/stderr instead of
// tool results — shell-capable agents skip the MCP schema/result overhead.
...
// Attach to a running browse-mcp browser (BROWSE_MCP_CDP) when possible;
// otherwise launch our own via the manager (profile + stealth + fence +
// proxy); ephemeral as the last resort when the profile is locked.
async function acquirePage(): Promise<PageSession> {
  if (existsSync(CDP_FILE)) {
    try {
      ...
      return {
        page,
        // close() on a connectOverCDP browser disconnects; it does not kill
