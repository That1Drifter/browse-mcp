...
// PDF text extraction via pdfjs-dist (legacy build works in Node without a
// worker). Loaded lazily so the dependency costs nothing until the first PDF.
...
export function isPdfData(data: Uint8Array): boolean {
  // %PDF magic bytes; some files have a small junk prefix, so scan the first 1KB.
  const head = Buffer.from(data.slice(0, 1024)).toString('latin1');
  return head.includes('%PDF-');
}

/** Fetch a PDF from an http(s) URL or read it from a local file path. */
